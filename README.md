...
1. **Frontend**: Creado con React, TypeScript y Vite. Utiliza la Context API y el hook `useReducer` para la gestión global y centralizada del estado. Se comunica con el backend mediante `axios` y ahora permite editar participantes reutilizando componentes.
2. **Backend**: Creado con FastAPI y Python. Utiliza SQLite como base de datos local y SQLModel (ORM) para la gestión y validación de datos, incorporando el método `PUT` para actualización de registros.

## Estructura del Proyecto

- `/frontend`: Código fuente de la interfaz de usuario en React, con la lógica de estado separada en un `reducer`.
