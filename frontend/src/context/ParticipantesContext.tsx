import { createContext, useReducer, useEffect, useContext, type ReactNode } from "react";
...
export function ParticipantesProvider({ children }: { children: ReactNode }) {
  ...
  const editar = async (id: number, p: Participante) => {
    try {
      const response = await axios.put(`${API_URL}/${id}`, p);
  ...
  const seleccionarParaEditar = (p: Participante | null) => {
